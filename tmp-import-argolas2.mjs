...
const DIR = "/tmp/argolas2";
const CATEGORY = "argola";
const CONCURRENCY = 6;
...
async function worker() {
  while (idx < files.length) {
    const i = idx++;
    try { await processFile(files[i]); ok++; }
    catch (e) { fail++; console.error(`FAIL ${files[i]}: ${e.message}`); }
    if ((ok+fail) % 25 === 0) console.log(`progress ${ok+fail}/${files.length} ok=${ok} fail=${fail}`);
  }
}
